  ...
  return (
    <div className="h-[calc(100%-9rem)] overflow-y-auto">
      {messages.map((message) => {
        const isAuthUserMessage = authUser && message.receiverId === authUser._id
        ...
        return (
          <div key={message._id} className={`chat chat-${isAuthUserMessage ? "end" : "start"}`}>
            ...
            <div className="chat-header">
              {userInfo?.name}
              <time className="text-xs opacity-50 mx-2">{message.createdAt.split("T")[1].slice(0, 6)}</time>
            </div>
            {message?.text && <div className="chat-bubble">{message.text}</div>}
            {message?.picture && <div className="chat-bubble">{message.picture}</div>}
          </div>
        )
      })}
