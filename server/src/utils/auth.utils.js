...
const handleError = (res, status, message) => {
  ...
  else return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ERROR_MESSAGES.SERVER_ERROR)
}

const validateSignUpData = async (name, email, password) => {
  if (!name || !email || !password) {
    throw new RequestError(ERROR_MESSAGES.MISSING_FIELDS, HTTP_STATUS.BAD_REQUEST)
  }

  if (password.length < 6) {
    throw new RequestError(ERROR_MESSAGES.PASSWORD_LENGTH, HTTP_STATUS.BAD_REQUEST)
  }
}
